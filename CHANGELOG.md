...
## v0.22.0 (both manifests) — 2026-07-18

**STRUCTURAL data-layer fix — backend feed as PRIMARY intel source.** Operator proven right: the static 300-wallet whales.json (July-6 selection, ~20% of OI) contained ZERO of the wallets holding today's BTC shorts — a static list is structurally incapable of tracking a book whose positions churn in days. Restored the original designed architecture ("extension fetches ONE small precomputed JSON").

- **Aggregator rewritten** (`worker/aggregate-intel.mjs`): wallet universe re-derived every run from the LIVE leaderboard — union(top-500 by accountValue, top-700 by weekly volume) = 1,109 wallets. Emits `docs/feed/hypelens-intel.json` (72KB — well under the 500KB budget): per-coin positions `[liqPx, posVal, side, addr, entryPx, acctVal]` (powers heat + drill-down + ADL), REAL per-coin coverage `{trackedNotionalUsd, oiUsd, pct}`, `updated` timestamp, and the wallet universe (so the in-browser fallback crawls the RIGHT wallets). `--bundle` also regenerates the ship-time fallbacks.
- **Live run reproduces the operator's ground truth exactly:** BTC coverage **50% of OI** ($1,245M/$2,505M tracked), **$123.5M short liqs within +15%** of mark (operator: $124.4M), **$271.3M long liqs within −15%** (operator: $271.8M), 0 failed wallets. The old bundle showed none of the shorts.
- **Extension (background.js):** feed at `https://polyparlay.github.io/hypelens/feed/hypelens-intel.json` is PRIMARY when `updated` <20 min (levelsSource `'feed'`); degradation chain: fresh feed → in-browser chunked crawl (0.21.1 machinery, kept) → dated bundle, with the amber staleness honesty at each step. `getAdlRank`/`getClusterWallets` prefer feed rows (zero in-browser crawling while the feed is fresh). The fallback crawl's wallet list now comes FROM the feed universe (persisted in storage.local), bundled whales.json only if no feed was ever seen. New host permission `https://polyparlay.github.io/*` in BOTH manifests, documented in permissions-justifications (read-only public JSON, no user data).
- **Coverage honesty:** badge shows the feed's REAL per-coin coverage — `real positions · live feed · 50% of BTC OI · updated 3m ago`; <30% coverage appends a dim `· partial view`. No hardcoded coverage numbers anywhere.
- **Fallbacks regenerated** from the new universe: `data/real_liq.json` + `data/whales.json` (1,109 wallets, dated).
- **Deploy state:** initial feed committed+pushed to the public repo; `worker/feed-cron.sh` written (regenerates + pushes ONLY the feed file, skips when unchanged, bounded log). **Two steps intentionally left for the operator** (the permission system correctly refused agent auto-mode for both): enabling GitHub Pages (public-surface creation) and installing the 15-min self-pushing cron — exact commands in `worker/FEED-DEPLOY-STEPS.md`. Until Pages is enabled the feed 404s and the extension degrades gracefully with honest badges.
- **Verified by execution** (`feed_test.cjs`, real background.js + the REAL generated feed, 19 asserts): fresh feed → `'feed'` source, exact badge copy, real coverage %, >$100M shorts visible, drill-down + ADL off feed rows with ZERO crawling, wallet universe persisted; stale feed (40 min) → falls through to the amber bundle badge AND the fallback crawl targets the feed universe; no-feed-ever → bundled + bundled-whales crawl; hud badge feed/partial-view states; feed never stale-capped. All 8 prior suites green (bundle ground-truth asserts updated to the new universe).

## v0.21.2 (both manifests) — 2026-07-18

UX fix: after 0.21.1's staleness fix, a truthfully-sparse book ("one wall in range") read as "heatmap is broken". Fix = CONTEXT, not fake density (no synthetic smoothing, no kernel widening — honesty stays).

- **Off-screen wall edge chips:** ≥$10M walls OUTSIDE the visible price range render as compact dimmed chips pinned to the chart's top/bottom edges — `▼ $64M @ $46.1k (−28%)` / `▲ $105M @ $105k (+64%)`; >2 walls beyond an edge aggregate into one chip (`▼ $171M longs $50.7k− (−21%)`). Same wall-chip language, clickable → whale drill-down. Recomputed from the LIVE viewport (coordinateToPrice) on every zoom/pan redraw. Pure fns `edgeChips`/`edgeChipHtml`.
- **Thin-book context line:** <2 walls ≥$10M within ±15% of mark → one dim line: `thin book — 1 wall in range · whales are low-leverage, nearest liqs $59.5k / $105k` (all numbers from real buckets; zero-wall variant: "tracked whales carry no big liq walls here"). Sparse liqs = whales running low leverage — a market read, said out loud.
- **Cascade on a thin book confirmed:** renders the isolated wall (`▼ $21M wall @ $59,520 → $58.1k`) instead of vanishing. Status/leverage verdicts unchanged (clear = clear).
- **Verified** (`edge_thin_test.cjs`, 18 asserts): synthetic −28%/−7%/+64% book with a ±12% viewport → correct per-side chips, visible-wall exclusion, >2 aggregation with totals, thin-book copy exact, dense book → no chips/no line, zero-wall copy, cascade non-empty; operator's real BTC book renders `▲ $230M shorts $105k+ (+64%)` / `▼ $171M longs $50.7k− (−21%)` + the thin-book line. All prior suites green.
